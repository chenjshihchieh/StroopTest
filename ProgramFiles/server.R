...
function(input, output){
  ...
  observeEvent(input$image1, {
    ...
    rv$endTime <- Sys.time()
    reactTime2$Time[rv$Qnumber-totQs] <<- as.numeric(rv$endTime - rv$initialTime)
    rv$Qnumber <- rv$Qnumber + 1
    rv$initialTime <<- Sys.time()
  ...
  observe({
    ...
    if(value == 1){
      ...
      reactTime <<- data.frame(Category = question_pool$Category, Time = rep(NA, totQs)) #Reaction Time data
  ...
  observe({
    ...
    if(value == 2 & rv$Qnumber > totQs){
      ...
      reactTime2 <<- data.frame(Category = question_pool$Category, Time = rep(NA, totQs)) #Reaction Time data
  ...
  output$reactTable4 <- renderTable({
    ...
    unused3 <- input$word4
    data.frame(
      conTime = mean(reactTime2$Time[reactTime2$Category == "Congruent"], na.rm = TRUE),
      inconTime = mean(reactTime2$Time[reactTime2$Category == "Incongruent"], na.rm = TRUE)
    )
  })
